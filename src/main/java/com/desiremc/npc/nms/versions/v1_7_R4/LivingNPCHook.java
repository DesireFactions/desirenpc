...
import com.desiremc.npc.nms.ILivingNPCHook;
import com.desiremc.npc.nms.versions.v1_7_R4.ai.NPCPath;
...
public class LivingNPCHook extends NPCHook implements ILivingNPCHook
{
    ...
    public LivingNPCHook(LivingNPC npc, Location toSpawn, EntityType type)
    {
        ...
        if (nmsEntity instanceof LivingHookable)
            ((LivingHookable) nmsEntity).setHook(this);
    }
    ...
    public void navigateTo(Location l) throws PathNotFoundException
    {
        ...
        if (path == null)
            throw new PathNotFoundException(l, getEntity().getLocation());
        getNpc().addTask(path);
    }
